...
export default function Post(props) {
  ...
  useEffect(() => {
    ...
    async function fetchStravaData() {
      const requestActivities = await axios.get(
        `${baseStravaUrl}${stravaDataUrl}?access_token=${stravaAccessToken}&per_page=10`,
        {
      ...
      const runData = requestActivities.data.map(
        ({ name, distance, start_date, elapsed_time, workout_type }) => ({
          name,
          distance,
          start_date,
          elapsed_time,
    ...
    async function fetchStravaUserData() {
      const requestUser = await axios
        .get(
          `${baseStravaUrl}/api/v3/athlete?access_token=${stravaAccessToken}`,
          {
            Authorization: `Bearer ${stravaAccessToken}`,
          }
        )
    ...
    async function fetchUserTotals() {
      const requestTotals = await axios
        .get(
          `${baseStravaUrl}/api/v3/athletes/${stravaUserId}/stats?access_token=${stravaAccessToken}`,
          {
            Authorization: `Bearer ${stravaAccessToken}`,
          }
        )
      ...
      window.localStorage.setItem(
        "stravaTotals",
        JSON.stringify(requestTotals.data.all_run_totals)
      );
      window.localStorage.setItem(
        "stravaYtdTotals",
        JSON.stringify(requestTotals.data.ytd_run_totals)
      );
    }
  ...
  useEffect(() => {
          ...
          }),
          {
            headers: {
              Authorization: `Basic ${Buffer.from(
                spotifyApiData.client_id + ":" + spotifyApiData.client_secret
              ).toString("base64")}`,
    ...
    if (
      props.spotifyAuthCode !== 1 &&
      props.spotLoggedIn &&
      props.spotifyStateMatch
    ) {
      authSpotify();
    }
    ...
    const fetchSpotifyData = async () => {
      ...
      // console.log(before);
      // console.log(after);
      const requestRecentlyPlayed = await axios
      ...
      window.localStorage.setItem(
        "SpotifyData",
        JSON.stringify(requestRecentlyPlayed.data.items)
      );
    ...
    const fetchSpotifyUserData = async () => {
      const requestUserInfo = await axios
      ...
      window.localStorage.setItem(
        "SpotifyUserName",
        requestUserInfo.data.display_name
      );
      window.localStorage.setItem(
        "SpotifyUserProfile",
        requestUserInfo.data.images[0].url
      );
    };
    ...
    const fetchTopItems = async () => {
      const requestTopArtists = await axios
        .get(
      ...
      const requestTopTracks = await axios
        .get(
  ...
  return (
    <div className="post-list">
      {localStorage.getItem("runData") && <Sidebar />}
